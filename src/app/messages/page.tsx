    ...
    <main className="min-h-screen bg-surface pb-32">
      <header className="sticky top-0 z-40 bg-surface/95 backdrop-blur-lg border-b border-outline-variant/20">
        <div className="flex items-center gap-3 px-5 py-4 max-w-md mx-auto">
          ...
          <button className="ml-auto text-primary" aria-label="New message">
      ...
      <div className="max-w-md mx-auto px-5 pt-4">
        ...
        <div className="divide-y divide-outline-variant/10">
          {threads.map((t) => (
            <button key={t.handle} className="w-full flex items-center gap-3 py-3 px-2 hover:bg-surface-container-low transition-colors text-left rounded-lg">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center justify-between mt-0.5">
                  <span className="text-sm text-on-surface-variant truncate">{t.last}</span>
                  {t.unread > 0 && (
                    <span className="ml-2 bg-on-tertiary-container text-white text-[10px] font-bold w-5 h-5 rounded-full flex items-center justify-center shrink-0">{t.unread}</span>
                  )}
                </div>
              </div>
            </button>
