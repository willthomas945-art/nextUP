...
export default function FeedPage() {
  ...
  const ordered = useMemo(() => {
    ...
    // Algorithmic boost: posts whose ID is in likedPostIds get pushed up; otherwise shuffle by hash
  ...
  return (
    <header className="absolute top-0 left-0 right-0 z-30 bg-gradient-to-b from-primary/70 via-primary/30 to-transparent">
      ...
      <div className="flex justify-center gap-1 pb-3">
        <button onClick={() => setGenreScope("all")} className={`label-caps text-[9px] px-3 py-1 rounded-full transition-colors ${genreScope === "all" ? "bg-white/15 text-white backdrop-blur-md border border-white/20" : "text-white/50"}`}>ALL GENRES</button>
        {hasMyGenres ? <button onClick={() => setGenreScope("my")} className={`label-caps text-[9px] px-3 py-1 rounded-full transition-colors ${genreScope === "my" ? "bg-white/15 text-white backdrop-blur-md border border-white/20" : "text-white/50"}`}>MY GENRES</button> : null}
      </div>
    </header>
  );
}
...
function FeedSlide({ post }: { post: Post }) {
  const { state, toggleFollow, toggleLike } = useStore();
  ...
  return (
    <section className="feed-slide relative h-screen w-full overflow-hidden text-white" style={{ background: post.gradient }} onClick={handleTap}>
      <div className="grain" />
      ...
      <div className="absolute right-3 bottom-32 flex flex-col items-center gap-5 z-20">
        <Link href="/boost" onClick={(e) => e.stopPropagation()} className="flex flex-col items-center gap-1 active:scale-90 transition-transform">
          <span className="w-12 h-12 bg-tertiary-container rounded-full flex items-center justify-center shadow-lg border border-on-tertiary-container/30">
            <svg className="w-6 h-6 text-on-tertiary-container fill-current" viewBox="0 0 24 24"><path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/></svg>
          </span>
      ...
      <div className="absolute left-5 bottom-24 max-w-[78%] z-20" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex items-center gap-3 mb-3">
          <Link href={`/artist/${post.artist.handle}`} className="block">
          ...
          <button onClick={(e) => { e.stopPropagation(); toggleFollow(post.artist.handle); }} className={`label-caps text-[10px] px-3 py-1 rounded-full border transition-all ${following ? "bg-tertiary-container border-on-tertiary-container/50 text-on-tertiary-container" : "bg-white/10 backdrop-blur-md border-white/30 text-white"}`}>
